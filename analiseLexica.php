...
class AnalisadorLexico {
    ...
    function lexer($sourceCode) {
        ...
        while ($i < $length) {
            ...
            // Identificar palavras e números
            if (ctype_alpha($sourceCode[$i])) { // Se começar com letra, é uma palavra ou identificador
                ...
                // Verifique primeiro se a palavra é uma palavra reservada
                if (isset($palavraReservadaDescricao[strtolower($word)])) {
                    $tokens[] = [
                        'PALAVRARESERVADA',
                        $word,
                        $palavraReservadaDescricao[strtolower($word)],
                        'linha' => $linha,
                        'coluna' => $coluna - strlen($word)
                    ];
                } elseif (isset($descricaoTipos[$word])) {
                    ...
                    // Caso contrário, trate como um identificador
                    $tokens[] = [
                        'IDENTIFICADOR',
                        $word,
                        'linha' => $linha,
            ...
            // Identificar operadores e símbolos
            if (isset($operadorDescricao[$sourceCode[$i]])) {
                $tokens[] = [
                    'OPERADOR',
                    $sourceCode[$i],
                    $operadorDescricao[$sourceCode[$i]],
                    'linha' => $linha,
                    'coluna' => $coluna
                ];
                $i++;
            ...
            // Verificar se é constante numérica
            if (ctype_digit($sourceCode[$i])) {
            ...
            // Se o token não for reconhecido, registre um erro
            $erros[] = "Erro léxico: token desconhecido '$sourceCode[$i]' na linha $linha, coluna $coluna";
            $i++;
            $coluna++;
        }
    
        $this->tokens = $tokens;
        return ['tokens' => $this->tokens, 'erros' => $erros];
    }
}    